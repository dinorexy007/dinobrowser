...
class BrowserProvider extends ChangeNotifier {
  ...
  double _splitRatio = 0.7; // 70% primary, 30% secondary for better video visibility
  ...
  Future<void> closeTab(int index) async {
    ...
    // Get the tab being closed for cleanup
    final tabToClose = workspaceTabs[index];
    
    // If this is the last tab, reset it to blank instead of removing
    if (workspaceTabs.length <= 1) {
      final tab = workspaceTabs.first;
      tab.url = '';  // Empty URL shows home screen
      tab.title = 'New Tab';
      tab.isLoading = false;
      tab.canGoBack = false;
      tab.canGoForward = false;
      tab.controller = null;  // Clear controller to force WebView recreation
      tab.keepAlive = null;   // Clear keepAlive for fresh start
      _workspaceCurrentIndices[workspaceId] = 0;
      debugPrint('[BrowserProvider] Last tab reset to home screen');
      notifyListeners();
      return;
    }
    
    // Clean up tab resources before removing
    tabToClose.controller = null;
    tabToClose.keepAlive = null;
    ...
    debugPrint('[BrowserProvider] Closed tab $index, ${workspaceTabs.length} tabs remaining');
    notifyListeners();
  }
