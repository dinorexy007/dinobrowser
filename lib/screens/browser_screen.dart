...
class _BrowserScreenState extends State<BrowserScreen>
  ...
  // PERSISTENT WebView cache - prevents recreation on tab switch
  ...
  Future<void> _initializeBrowser() async {
    ...
    // Show welcome popup for first-time users (one-time only)
    if (mounted) {
      WelcomePopup.showIfNeeded(context);
    }
  }
      ...
      builder: (context, provider, authProvider, child) {
        ...
        return PopScope(
          ...
          onPopInvokedWithResult: (didPop, result) async {
            ...
            final isOnHomeScreen = currentTab == null || currentTab.url.isEmpty || currentTab.url == 'about:blank';
            
            debugPrint('[Back Button] Tabs: ${tabs.length}, Current tab has controller: ${currentTab?.controller != null}, isOnHomeScreen: $isOnHomeScreen');
            
            // Priority 1: Try to go back in current WebView history
            if (currentTab?.controller != null) {
              final controller = currentTab!.controller!;
              final canGoBack = await controller.canGoBack();
              debugPrint('[Back Button] Can go back in WebView: $canGoBack');
              
              if (canGoBack) {
                await provider.goBack();
                debugPrint('[Back Button] Went back in WebView history');
                return;
              }
            }
            
            // Priority 2: If multiple tabs exist, close current tab and go to previous
            if (tabs.length > 1) {
              debugPrint('[Back Button] Multiple tabs (${tabs.length}), closing current tab');
              final currentIndex = provider.currentTabIndex;
              await provider.closeTab(currentIndex);
              return;
            }
            
            //Priority 3: If on home screen with single tab, show exit confirmation
            if (isOnHomeScreen && tabs.length == 1) {
              debugPrint('[Back Button] On home screen with single tab, showing exit dialog');
              _showExitConfirmationDialog();
              return;
            }
            
            // Priority 4: If single tab with content but no history, close it to show home screen
            if (tabs.length == 1 && !isOnHomeScreen) {
              debugPrint('[Back Button] Single tab with no history, closing to show home screen');
              await provider.closeTab(0);
              return;
            }
            
            // Priority 5: Last resort - close current tab
            debugPrint('[Back Button] Default: closing current tab');
            final currentIndex = provider.currentTabIndex;
            await provider.closeTab(currentIndex);
          },
          child: Scaffold(
              ...
              child: Column(
                children: [
                  ...
                  Expanded(
                    child: Stack(
                      children: [
                        // Main content - WebView uses Selector to prevent rebuilds
                        showHomeScreen
                                ...
                                : provider.isSplitScreen
                                    ...
                                    : Selector<BrowserProvider, int>(
                                        ...
                                        builder: (context, currentIndex, child) {
                                          return _buildSingleWebView(context.read<BrowserProvider>());
                                        },
                                      ),
  ...
  /// Build WebViews for all tabs - CACHED to prevent recreation
  Widget _buildSingleWebView(BrowserProvider provider) {
    // If no tabs exist, return empty container
    if (provider.tabs.isEmpty) {
     ...
     // Clean up caches for deleted tabs
    final tabIds = provider.tabs.map((t) => t.id).toSet();
    _webViewCache.removeWhere((key, _) => !tabIds.contains(key));
    _webViewKeys.removeWhere((key, _) => !tabIds.contains(key));
    
    // Build WebViews - use GlobalKey to preserve state
    final webViews = <Widget>[];
    for (final tab in provider.tabs) {
      // Get or create GlobalKey for this tab
      _webViewKeys.putIfAbsent(tab.id, () => GlobalKey());
      final key = _webViewKeys[tab.id]!;
      
      // Only create WebView widget if not cached
      if (!_webViewCache.containsKey(tab.id)) {
        final tabUrl = (tab.url.isEmpty || tab.url == 'about:blank')
            ? 'https://www.google.com'
            : tab.url;
            
        _webViewCache[tab.id] = KeyedSubtree(
          key: key,
          child: WebViewStack(
            initialUrl: tabUrl,
            isPrimary: true,
            ...
            onPageFinished: (url, title) {
              if (tab.id == provider.currentTab?.id) {
                _checkBookmark();
              }
            },
          ),
      ...
      webViews.add(_webViewCache[tab.id]!);
    }
    
    // Handle pending URL navigation via controller (not recreating WebView)
    if (_pendingUrl != null && provider.currentTab?.controller != null) {
      final url = _pendingUrl!;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        provider.currentTab?.controller?.loadUrl(
          urlRequest: URLRequest(url: WebUri(url)),
        );
        if (mounted) setState(() => _pendingUrl = null);
      });
    }
    
    // IndexedStack keeps all mounted, only shows current
    return IndexedStack(
  ...
  Widget _buildSplitScreen(BrowserProvider provider) {
    // Get the current tab's URL - use existing if available
    final primaryUrl = provider.currentTab?.url ?? 'https://www.google.com';
    final primaryTabId = provider.currentTab?.id ?? 'main';
    
    return Column(
      children: [
        // Primary WebView - reuses existing tab
        Expanded(
          ...
          child: Container(
            ...
            child: WebViewStack(
              key: ValueKey('split_primary_$primaryTabId'),
              initialUrl: primaryUrl,
              isPrimary: true,
              onControllerCreated: (controller) {
                provider.currentTab?.controller = controller;
              },
            ),
          ),
        ),
          ...
          child: Stack(
            children: [
              WebViewStack(
                ...
                keepAlive: provider.secondaryTab?.keepAlive, // Preserve state
                isPrimary: false,
                onControllerCreated: (controller) {
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  const Spacer(),
                  // Add to Workspace button - hide in General workspace
                  if (authProvider.isLoggedIn && 
                      provider.currentWorkspace.name != 'General' &&
                      (provider.currentTab != null && !provider.currentTab!.isBlank))
                    Flexible(
                      child: IconButton(
                        onPressed: () {
                          Navigator.pop(context);
